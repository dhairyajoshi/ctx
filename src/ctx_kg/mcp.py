...
class McpServer:
    ...
    def tool_semantic(self, args: dict[str, Any]):
        ...
        return self.with_store(lambda store: self._semantic_query(store, query, limit, provider_name, model))

    def _semantic_query(self, store: GraphStore, query: str, limit: int, provider_name: str | None = None, model: str | None = None):
    ...
    def tool_status(self, args: dict[str, Any]):
        def query(store: GraphStore):
            ...
            current = _current_commit_safe(self.config.repo)
            counts = store.counts()
            return {
    ...
    return [
        {
            ...
            "inputSchema": object_schema({"query": "string", "limit": "number", "fallback": "boolean"}, ["query"]),
        },
        {
            "name": "ctx_semantic",
